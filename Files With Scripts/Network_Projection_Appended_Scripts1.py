...
class run:
    ...
    def plot_sphere_a(self, sph):
        ...
        self.ax.view_init(elev=9., azim=175)
...
#EVERYTHING THAT IS NOT COMMENTED IS PROBABLY NECESSARY FOR ALGORITHMS TO WORK
...
########################################################################
"""
#Using Pickled files for connections, Uncomment for use. 
#opening pickled files for reinstatement of connections

s.construct_icosphere() #Making sphere
...
#Defining connections, comment pickled data script above within #######, if being used. 

#conn = Define_Connections(s) #Defining class that defines connections and constructs sphere if you don't want to use pickled data
#colours, vconn, hconn, pent_ind = conn.define_connections() # Defining connections, not needed if using pickled data
...
#colours is any 1d array that is the number of nodes. All values must be less than 1. 
...
#####################################################################
#To plot Sphere After propagation.


#runc = run(network = n, plot=False,store=False,runs=5,fib_threshold=350)
#runc.propagate_storage()
#colours = runc.network.nodes
#colours = colours/sum(colours) #normalising so all values less than 1
#runc.plot_sphere_a(s)
#runc.surf.set_array(colours)
#runc.ax.view_init(elev=0., azim=145) #Setting the viewing angle
#plt.show()
...
#conn.connectome() #visualisation of connections, as you've seen, need to define conn class first
...
###################################################################
#Animation and storage of animation
...
#runc.anim1.save('SphereNu0245paroxrun2.mp4', fps=25, extra_args=['-vcodec', 'libx264']) #change file name as necessary
#plt.show()
