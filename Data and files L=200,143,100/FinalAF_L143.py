...
class run: #Class to run code
    ...
    def storesteps(self):
        ...
        for elements in range(self.stepsstored):
            ...
            if len(self.heart.grid[self.heart.grid == self.heart.excitation]) > 160:
                self.fibrillation()

            elif len(self.heart.grid[self.heart.grid==self.heart.excitation]) <= 160 and self.infibrillation == True:
                if len(self.num_excited) > 220 and all(i <= 160 for i in self.num_excited[-221:]) :
...
h = heart(L=143,p_unexcitable=0.05,p_fibrosis= 0.4,p_dysf=0.05)
#h.electrocardiosetup([100,100])
r = run(heart=h, plot=True,store=False,stepsstored=100,replot=False)
#r.storesteps()
