...
class heart:
    def __init__(self,L=100,p_unexcitable=0.05,p_transv= 0.25,p_dysf=0.05):
	...
	"""
        ...
        self.tempgrid+=self.heartbeatsteps
...
class run: #Class to run code
    ...
    def storesteps(self):
        ...
        for elements in range(self.stepsstored):
            ...
            if self.num_excited[-1] > self.heart.L + 20:
                self.fibrillation()

            elif self.num_excited[-1] <= (self.heart.L + 20) and self.infibrillation == True:
            ...
            #if self.num_excited[-1] >143:
            #    self.fibrillation()

            #elif self.num_excited[-1] <= 143 and self.infibrillation == True:
            #    if len(self.num_excited) > 220 and all(i <= 160 for i in self.num_excited[-441:]) : #THIS IS QUADRUPLE PERIOD
            #        self.stopfibrillation()
