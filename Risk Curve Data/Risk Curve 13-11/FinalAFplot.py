    ...
    def plot_grid(self):
        for elements in range(self.stepsstored):
            if (self.heart.time % self.heart.heartbeatsteps)==0 and self.heart.time!=0:     #why self.heart.time != 0??
                self.heart.excitecolumn()
            self.heart.onestep()
        self.figi = plt.figure()
        self.ax1 =self.figi.add_subplot(111)
            
            
        self.ax1.set_title('t = 470, Nu = 0.15' )
        self.ax1.set_xlabel('x')
        self.ax1.set_ylabel('y')
                
        
        self.im = self.ax1.imshow(self.heart.grid, extent=(-0,self.heart.L, self.heart.L, 0), aspect = 'auto', cmap = "Greys_r")
        self.im.set_cmap('Greys_r') 
...
""" 
h = heart(L=200,p_unexcitable=0.05,p_fibrosis= 0.1,p_dysf=0.05)

r = run(heart=h, plot=False,store=False,stepsstored=690,replot=False)
ima = r.plot_grid()
#r.plotecg()
plt.show()