...
class heart:
    def __init__(self,L=143,p_unexcitable=0.05,p_fibrosis= 0.83,p_dysf=0.05):
	...
	"""
        ...
        self.excitation=35 #Value of excitation on the lattice
        self.heartbeatsteps=157 #Time period between excitation wavefronts
...
class run: #Class to run code
    ...
    def storesteps(self):
        ...
        for elements in range(self.stepsstored):
            if (self.heart.time % self.heart.heartbeatsteps)==0:    
                self.heart.excitecolumn()
            self.heart.onestep()
            #self.electrocardiot.append(self.heart.electrocardio())
            self.num_excited.append(len(self.heart.grid[self.heart.grid == self.heart.excitation]))
            ...
            if len(self.heart.grid[self.heart.grid == self.heart.excitation]) > self.heart.L + 20:
                self.fibrillation()

            elif len(self.heart.grid[self.heart.grid==self.heart.excitation]) <= (self.heart.L + 20) and self.infibrillation == True:
                if len(self.num_excited) > 2*self.heart.heartbeatsteps  and all(i <= (self.heart.L + 20) for i in self.num_excited[-(2*self.heart.heartbeatsteps+1):]) :
    ...
    def plot_grid(self):
        ...
        self.im.set_cmap('Greys_r') 
        self.im.set_array(self.heart.grid)
        self.ax1.set_title('Square Lattice - Nu=%.2f - t=%s'%(1-self.heart.p_fibrosis,self.heart.time))
        self.ax1.set_xlabel('x')
        self.ax1.set_ylabel('y')
...
h = heart(200,0.05,1,0.05)
#h.electrocardiosetup([100,100])
r = run(heart=h, plot=False,store=False,stepsstored=150,replot=False)
r.excitation=50 #Value of excitation on the lattice
r.heartbeatsteps=220 #Time period between excitation wavefronts
r.storesteps()
...
timex = range(r.stepsstored)
ax2.plot(timex, r.num_excited )
#ax2.view_init(elev=10, azim=140)
#ax2.set_xlim(-0.7, 0.7)
#ax2.set_ylim(-0.7, 0.7)
#ax2.set_zlim(-0.7, 0.7)
ax2.set_title('Number of Excited Cells - No Fibrillation - Nu=0.25')
ax2.set_xlabel('Time (timesteps)')
ax2.set_ylabel('Number of excited cells')
#ax2.set_zlabel('z')
"""
plt.show()
"""
